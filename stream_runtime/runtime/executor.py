...
class NodeExecutor:
    ...
    def execute(self, node, x, loaded, plan):
        if self.trace:
            self.trace(f"[EXEC] Node {node.id} {node.type} strategy={plan.strategy}")
        if node.type == "ReLU":
            return torch.relu(x)
        if node.type == "Add":
            return x + x
        if node.type == "Embedding":
            name = next(n for n in node.weights if n.endswith(".weight"))
            w = self.store.tensor(name).to_numpy()
            return torch.nn.functional.embedding(x.long(), torch.from_numpy(w))
