...
class SyncScheduler:
    def run(self, engine, value):
        return engine.run(value)


class AsyncScheduler:
    def __init__(self, store, manager, trace=None):
        self.store, self.mm, self.trace = store, manager, trace

    async def prefetch(self, node):
        required = sum(self.store.metadata(n).nbytes for n in node.weights)
        if required > self.mm.available:
            return None
        if self.trace:
            self.trace(f"[PREFETCH] Node {node.id} admitted={required} bytes")
    ...
    async def run(self, engine, value):
        ...
        for i, node in enumerate(engine.graph.nodes):
            if i + 1 < len(engine.graph.nodes):
                await self.prefetch(engine.graph.nodes[i + 1])
