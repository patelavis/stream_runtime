...
def prepared(tmp_path):
    p, _ = model(tmp_path)
    from stream_runtime.storage import SafeTensorStream

    r = SafeTensorStream(p)
    nodes, tensors = ArchitectureAdapter().analyze(r)
    d = tmp_path / "prepared"
    d.mkdir()
    ModelGraph("generic", "generic", nodes, tensors).save(d / "manifest.json")
    (d / "model.path").write_text(p)
    return d


def test_local_offline_api(tmp_path):
    d = prepared(tmp_path)
    app = create_app(d, 256, api_key="local")
    ...
    async def run():
        ...
        async with httpx.AsyncClient(transport=transport, base_url="http://test") as c:
            ...
            st = (await c.get("/v1/status")).json()
            assert st["peak_ram"] <= 256 and st["model_on_disk"]
